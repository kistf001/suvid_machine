...
void fdfddd(){
    ...
    if(digitalRead(inPin)) dfdf0++;
    else dfdf0 = 0;
    if(digitalRead(inPin)) dfdf1++;
    else dfdf1 = 0;
    if(digitalRead(inPin)) dfdf2++;
    else dfdf2 = 0;

    if ( threshold < dfdf0 ) state0 = 1;
    else state0 = 0;
    if ( threshold < dfdf1 ) state1 = 1;
    else state1 = 0;
    if ( threshold < dfdf2 ) state2 = 1;
    else state2 = 0;
...
ISR(ADC_vect) {
  ...
  alert_temp_cheack();
  if (alert != 0) state_pin = 0, digitalWrite( 5, LOW);
